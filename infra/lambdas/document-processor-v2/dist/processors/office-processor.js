...
exports.OfficeProcessor = void 0;
...
class OfficeProcessor {
    ...
    async processDocx(buffer) {
        const logger = (0, lambda_logger_1.createLambdaLogger)({ operation: 'OfficeProcessor.processDocx' });
    ...
    async processXlsx(buffer) {
        const logger = (0, lambda_logger_1.createLambdaLogger)({ operation: 'OfficeProcessor.processXlsx' });
        ...
        workbook.SheetNames.forEach((sheetName, index) => {
            ...
            sheetData.push({
                ...
                columnCount: json.length > 0 ? Math.max(...json.map((row) => Array.isArray(row) ? row.length : 0)) : 0,
            });
        });
        return {
    ...
    async processPptx(buffer) {
        const logger = (0, lambda_logger_1.createLambdaLogger)({ operation: 'OfficeProcessor.processPptx' });
     ...
     */
    async extractTextFromSlideXml(slideXml, slideNumber) {
        return new Promise((resolve) => {
            (0, xml2js_1.parseString)(slideXml, { explicitArray: false, ignoreAttrs: true }, (err, result) => {
                const logger = (0, lambda_logger_1.createLambdaLogger)({ operation: 'OfficeProcessor.extractTextFromSlideXml' });
                if (err) {
    ...
    async convertToMarkdown(extractedContent, docType) {
    ...
    convertDocxToMarkdown(content) {
        ...
        if (content.html) {
            try {
                ...
                const logger = (0, lambda_logger_1.createLambdaLogger)({ operation: 'OfficeProcessor.convertDocxToMarkdown' });
    ...
    convertPptxToMarkdown(content) {
        ...
        if (content.slides && Array.isArray(content.slides)) {
            ...
            const sections = text.split(/## Slide \d+/).filter((section) => section.trim().length > 0);
    ...
    async chunkText(text) {
        ...
        const logger = (0, lambda_logger_1.createLambdaLogger)({ operation: 'OfficeProcessor.chunkText' });
...
exports.OfficeProcessor = OfficeProcessor;
