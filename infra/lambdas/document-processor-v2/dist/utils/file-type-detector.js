...
const lambda_logger_1 = require("./lambda-logger");
/**
...
class FileTypeDetector {
     ...
     */
    static detectFileType(buffer, fileName, mimeType) {
        const logger = (0, lambda_logger_1.createLambdaLogger)({ operation: 'FileTypeDetector.detectFileType' });
     ...
     */
    static detectByMagicNumber(buffer, fileName) {
        if (buffer.length < 4) {
            return {
                detectedType: 'unknown',
                confidence: 'low',
     ...
     */
    static detectOfficeTypeFromZip(buffer, fileName) {
        try {
            ...
            if (fileName) {
                ...
                if (ext.includes('.xlsx') || ext.includes('.xls'))
                    return 'xlsx';
                if (ext.includes('.pptx') || ext.includes('.ppt'))
                    return 'pptx';
            }
        }
        catch (error) {
            const logger = (0, lambda_logger_1.createLambdaLogger)({ operation: 'FileTypeDetector.detectOfficeTypeFromZip' });
     ...
     */
    static detectOfficeByContent(buffer) {
        try {
            ...
            const logger = (0, lambda_logger_1.createLambdaLogger)({ operation: 'FileTypeDetector.detectOfficeByContent' });
...
FileTypeDetector.MAGIC_NUMBERS = {
    // PDF signature
...
FileTypeDetector.OFFICE_CONTENT_TYPES = {
    docx: [
        'word/document.xml',
        'word/'
    ],
...
FileTypeDetector.EXTENSION_MAP = {
    '.pdf': 'pdf',
    '.docx': 'docx',
...
FileTypeDetector.MIME_TYPE_MAP = {
    'application/pdf': 'pdf',
