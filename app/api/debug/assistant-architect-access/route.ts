...
export async function GET() {
  try {
    ...
    // 2. Check for assistant-architect tool
    const toolResults = await db
      .select()
      .from(toolsTable)
      .where(
        and(
          eq(toolsTable.identifier, "assistant-architect"),
    ...
    if (!tool) {
      return NextResponse.json({ 
        error: "assistant-architect tool not found or not active",
        userId,
        hasAccess: false 
      }, { status: 404 })
    }
    ...
    const roleToolResults = await db
      ...
      .where(
        and(
          ...
          roleToolsTable.roleId.in(roleIds)
        )
      )
