 ...
 * GET /api/chat/jobs/[jobId] - Poll job status and get progressive updates
 * DELETE /api/chat/jobs/[jobId] - Cancel running job
 * 
 * This endpoint enables universal polling for all AI requests, overcoming
...
export async function GET(
  ...
  try {
    ...
    if (!job) {
      log.warn('Job not found', { jobId, userId: currentUser.data.user.id });
    ...
    if (job.userId !== currentUser.data.user.id) {
      log.warn('Job access denied - wrong user', { 
        jobId, 
    ...
    log.debug('Job found and authorized', {
      jobId,
      status: job.status,
      userId: job.userId,
...
export async function DELETE(
  ...
  try {
    ...
    const job = await jobManagementService.getJob(jobId);
    if (!job) {
      log.warn('Job not found for cancellation', { jobId, userId: currentUser.data.user.id });
      timer({ status: 'error', reason: 'job_not_found' });
      return new Response(JSON.stringify({
        error: 'Job not found',
        jobId,
    ...
    // 4. Verify job ownership
    if (job.userId !== currentUser.data.user.id) {
      log.warn('Job cancellation denied - wrong user', { 
        jobId, 
        jobUserId: job.userId,
        requestUserId: currentUser.data.user.id 
      });
    ...
    if (cancelled) {
      ...
      log.warn('Job cancellation failed - no rows updated', { jobId });
    ...
    log.error('Job cancellation error', { 
      jobId,
      error: error instanceof Error ? {
        message: error.message,
        name: error.name,
